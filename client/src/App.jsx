    ...
    <div className="min-h-screen bg-slate-50 flex font-sans text-slate-900 overflow-hidden">
      ...
      <motion.aside animate={{ width: showHistory ? "320px" : "80px" }} className="h-screen bg-white border-r border-slate-200 flex flex-col shadow-xl z-20">
        <div className="p-6 flex items-center justify-between">
          ...
          <button onClick={() => setShowHistory(!showHistory)} className="p-3 rounded-2xl bg-slate-50 text-emerald-600 mx-auto transition-all">
        ...
        <div className="flex-1 overflow-y-auto px-4 space-y-3">
          {showHistory && (history.length > 0 ? history.map((item, i) => (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} key={i} className="p-4 rounded-2xl bg-white border border-slate-100 shadow-sm text-xs">
              ...
              <p className="text-slate-500 truncate">{item.symptoms.join(', ')}</p>
            </motion.div>
          )) : <div className="text-center py-10 opacity-20 text-[10px] font-bold">NO RECORDS FOUND</div>)}
        </div>
      </motion.aside>
      ...
      <main className="flex-1 h-screen overflow-y-auto p-6 lg:p-12">
        <div className="max-w-5xl mx-auto pb-20">
          <nav className="flex justify-between items-center mb-10">
            ...
            <button onClick={handleLogout} className="p-3 rounded-2xl text-slate-400 hover:text-red-500 transition-all">
            ...
            <div className="lg:col-span-8 space-y-6">
              ...
              <div className="bg-white p-8 rounded-[2.5rem] shadow-sm grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="number" placeholder="24" className="w-full px-4 py-3 bg-slate-50 rounded-xl outline-none border border-transparent focus:border-emerald-200" 
                ...
                <div>
                  ...
                  <select className="w-full px-4 py-3 bg-slate-50 rounded-xl outline-none border border-transparent focus:border-emerald-200"
              ...
              <div className="bg-white p-8 rounded-[2.5rem] shadow-sm relative">
                ...
                <div className="relative mb-6">
                  ...
                  <input 
                    ...
                    className="w-full pl-12 pr-4 py-4 bg-slate-50 rounded-2xl outline-none border-2 border-transparent focus:border-emerald-100 transition-all"
                    value={searchTerm} 
                      ...
                      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
                        ...
                        {filteredSymptoms.map(s => (
                          <button key={s} onClick={() => toggleSymptom(s)} className="w-full text-left px-4 py-3 hover:bg-emerald-50 rounded-xl flex items-center justify-between group">
                            <span className="text-slate-700 font-medium capitalize">{s.replace(/_/g, ' ')}</span>
                ...
                <div className="flex flex-wrap gap-2 p-6 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200 min-h-[100px] content-start">
                  {selectedSymptoms.length === 0 ? (
                    <div className="flex flex-col items-center justify-center w-full text-slate-300 py-4 opacity-40">
                        ...
                        <p className="text-[10px] font-bold uppercase tracking-widest">Selected symptoms appear here</p>
                    </div>
                ...
                <button onClick={handleDiagnose} disabled={selectedSymptoms.length === 0 || loading}
                  className="w-full mt-10 py-5 bg-emerald-500 text-white rounded-[2rem] font-black text-xl hover:bg-emerald-600 shadow-lg transition-all flex items-center justify-center gap-3">
              ...
              <AnimatePresence mode="wait">
                {prediction ? (
                  <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
                    ...
                    <p className="text-slate-400 uppercase text-[10px] font-black mb-2 tracking-widest opacity-60">Analysis Complete</p>
                  ...
                  <div className="opacity-20 flex flex-col items-center">
                    ...
                    <p className="font-black uppercase tracking-[0.2em] text-xs">Waiting for symptoms</p>
                  </div>
                )}
              </AnimatePresence>
            </div>
